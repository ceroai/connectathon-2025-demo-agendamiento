...
from fhir import solicitar_cita, obtener_ultima_cita
...
# Maximum conversation history to keep (in minutes)
...
def cleanup_old_conversations():
    ...
    for phone_number, messages in conversations.items():
        if messages:
            ...
            if last_message_time and (current_time - last_message_time) > timedelta(minutes=CONVERSATION_TIMEOUT):
    ...
    if incoming_msg:
        ...
        match ai_response:
            case "AGENDAR":
                ...
                if response.ok:
                    resp.message("Estamos pidiendo la cita, te avisaremos cuando esté agendada")
                    ...
                    resp.message("Hubo un error al agendar la cita, envía un correo a ministra@minsal.cl")
            case "REASIGNAR":
            ...
            case str() if ai_response.startswith("REASIGNAR "):
                try:
                    ...
                    resp.message(f"Estamos reasignando tu cita para el {fecha}, te confirmaremos cuando esté lista")
                except IndexError:
