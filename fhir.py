...
with open('body_solicitar_cita.json', 'r') as file:
...
def get_solicitar_cita_body(json_content: dict, patient_id: str) -> dict:
    ...
    for entry in content['entry']:
        ...
        if 'request' in entry and '{patientId}' in entry['request']['url']:
            entry['request']['url'] = entry['request']['url'].replace('{patientId}', patient_id)
        ...
        if 'resource' in entry:
            ...
            if 'subject' in resource:
                if '{patientId}' in resource['subject']['reference']:
                    resource['subject']['reference'] = resource['subject']['reference'].replace('{patientId}', patient_id)
            ...
            if 'beneficiary' in resource:
                if '{patientId}' in resource['beneficiary']['reference']:
                    resource['beneficiary']['reference'] = resource['beneficiary']['reference'].replace('{patientId}', patient_id)
...
def solicitar_cita(patient_id: str) -> requests.Response:
    ...
    access_token = get_access_token("https://auth.onfhir.cl/realms/conectaton-dev/protocol/openid-connect/token", client_id, client_secret)

    return requests.post("https://gateway.onfhir.cl/hl7cl/fhir/", headers={"Authorization": f"Bearer {access_token}"}, json=get_solicitar_cita_body(body_solicitar_cita_str, patient_id))
...
def obtener_ultima_cita(patient_rut: str = "99.999.999-9") -> Appointment:
    ...
    params = {
        "status": "booked",
        "patient.identifier": patient_rut
    }

    response = requests.get(f"https://gateway.onfhir.cl/hl7cl/fhir/Appointment", headers={"Authorization": f"Bearer {access_token}"}, params=params)
